...
void *br_alloc(size_t n)
{
    void *p = malloc(n ? n : 1);
    if (!p) {
        fputs("fbbridge: memoria insuficiente\n", stderr);
        exit(2);
    }
    return p;
}
    ...
    while (p < end) {
        ...
        if (eq) {
            ...
            if (kl < sizeof key) {
                ...
                else if (strcmp(key, "USER") == 0) set_field(&req->user, val, vl);
                else if (strcmp(key, "PASS") == 0) set_field(&req->pass, val, vl);
                else if (strcmp(key, "DSN") == 0) set_field(&req->dsn, val, vl);
...
{
    size_t i;
    free(req->action); free(req->user); free(req->pass); free(req->dsn);
...
char *br_clean_sql(const char *sql)
{
    ...
    while (n && isspace((unsigned char)sql[n - 1]))
        n--;
    /* em um bloco PSQL (EXECUTE BLOCK ...) o ';' final faz parte do bloco; ja
     * "EXECUTE PROCEDURE ...;" nao e um bloco e o ';' final deve ser removido */
    if ((n >= 5 && starts_with_ci(sql, "BEGIN")) || (n >= 7 && starts_with_ci(sql, "DECLARE")) ||
        (n >= 13 && starts_with_ci(sql, "EXECUTE BLOCK")))
