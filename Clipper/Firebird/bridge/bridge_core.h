/*
 * bridge_core.h - parte da FBBRIDGE que nao depende do Firebird.
 *
 * Le a requisicao do Clipper (.REQ), converte binds e paginas de codigo e grava o
 * DBF (dBase III) do resultado. Fica separada de fbbridge.c para poder ser
 * testada sem banco de dados (test_bridge.c).
 *
 * Todas as strings em memoria sao UTF-8 (a conexao com o Firebird usa charset
 * UTF8); a pagina de codigo do Clipper (cp850 etc.) so existe nas bordas: ao
 * ler arquivos e ao gravar o DBF/.RSP.
 */
...
/* Remove o ';' final, que o Firebird rejeita fora de blocos PSQL (novo buffer). */
 ...
 */
typedef struct {
    char *name; /* nome da coluna no Firebird (UTF-8) */
    ColKind kind;
    char **vals;
} Column;
