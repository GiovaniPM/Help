/*
 * bridge_core.h - parte da SQLITEBRIDGE que nao depende do SQLite.
 *
 * Le a requisicao do Clipper (.REQ), converte binds e paginas de codigo e grava o
 * DBF (dBase III) do resultado. Fica separada de sqlitebridge.c para poder ser
 * testada sem banco de dados (test_bridge.c).
 *
 * Todas as strings em memoria sao UTF-8 (e o formato nativo do SQLite); a pagina de
 * codigo do Clipper (cp850 etc.) so existe nas bordas: ao ler arquivos e ao gravar o DBF/.RSP.
 */
...
typedef struct {
    char *action, *file, *sqlfile, *rsp, *out;
...
/* Remove espacos nas bordas (novo buffer). O SQLite ignora ';' final, entao nao precisa remove-lo. */
 ...
 */
typedef struct {
    char *name; /* nome da coluna no banco (UTF-8) */
    ColKind kind;
    char **vals;
} Column;
