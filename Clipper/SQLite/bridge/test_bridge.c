 ...
 *   test_bridge <SQLITEBRIDGE.EXE>    roda o executavel com um arquivo .db inacessivel e confere o "ERR"
 */
...
{
    char *path = make_temp_path("sqlitebridge_t1.dbf"), msg[256];
...
{
    char *path = make_temp_path("sqlitebridge_t2.dbf"), msg[256];
...
{
    char *path = make_temp_path("sqlitebridge_t3.dbf"), msg[256];
    Dbf d;
    char *f;
    ...
    {   /* formatos que o SQLite devolve: ".5", expoente, negativos */
...
{
    char *path = make_temp_path("sqlitebridge_t4.dbf"), msg[256];
    Dbf d;
    char *f;
    char longo[301];
...
static void test_requisicao(void)
{
    ...
    Request req;
    char msg[128];
    /* CODEPAGE depois do FILE com acento: a pagina vem no proprio arquivo */
    const char *txt =
        "ACTION=QUERY\r\nFILE=C:\\dados\\banco jo\xc6o.db\r\nCODEPAGE=cp850\r\n"
        "MAXROWS=65000\r\nRSP=C:\\T\\SQL1.RSP\r\nOUT=C:\\T\\SQL1.DBF\r\nSQLFILE=C:\\T\\SQL1.SQL\r\n"
        "BIND=C|a\\nb\r\nBIND=N|5.0000000000\r\nBIND=D|20260921\r\nBIND=L|T\r\nBIND=Z|\r\n\x1a";
    ...
    CHECK_STR(req.file, "C:\\dados\\banco jo\xc3\xa3o.db"); /* convertido de cp850 para UTF-8 */
    CHECK_STR(req.rsp, "C:\\T\\SQL1.RSP");
    CHECK_STR(req.out, "C:\\T\\SQL1.DBF");
    CHECK_STR(req.sqlfile, "C:\\T\\SQL1.SQL");
...
static void test_sql(void)
{
    ...
    s = br_clean_sql("  select 1 ; \r\n"); CHECK_STR(s, "select 1 ;"); free(s);
    s = br_clean_sql("\r\n  select * from t\r\n"); CHECK_STR(s, "select * from t"); free(s);
}
...
/* Roda a ponte com um .db numa pasta inexistente: tem que sair com 0 e escrever ERR no .RSP. */
static int test_exe(const char *exe)
{
    char *req = make_temp_path("SQLT1.REQ"), *rsp = make_temp_path("SQLT1.RSP");
    ...
    if (!f)
        return 1;
    fprintf(f, "ACTION=PING\r\nFILE=./__pasta_inexistente_xyz__/nada.db\r\nCODEPAGE=cp850\r\nRSP=%s\r\n\x1a", rsp);
    fclose(f);
    remove(rsp);
